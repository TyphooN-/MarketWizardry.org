...
def extract_flavor_text(html_content):
    """Extract existing flavor text from gallery HTML"""
    if 'flavor-text">' in html_content:
        start = html_content.find('flavor-text">') + len('flavor-text">')
        end = html_content.find('</div>', start)
        if start > 0 and end > start:
            flavor_text = html_content[start:end].strip()
            # Clean up any HTML entities or extra formatting
            flavor_text = flavor_text.replace('\n', ' ').replace('  ', ' ').strip()
            return flavor_text
    return None

def create_simple_gallery(artist_name, preserve_flavor=True):
    ...
    if preserve_flavor and os.path.exists(existing_file):
        with open(existing_file, 'r', encoding='utf-8') as f:
            content = f.read()
            extracted_flavor = extract_flavor_text(content)
            if extracted_flavor:
                flavor_text = extracted_flavor
...
def convert_all_galleries():
    """Convert all existing galleries to simple format"""
    
    nft_gallery_dir = "/home/typhoon/git/MarketWizardry.org/nft-gallery"
    
    if not os.path.exists(nft_gallery_dir):
        print(f"NFT gallery directory not found: {nft_gallery_dir}")
        return
    
    # Find all artist directories with webp folders
    artists = []
    for item in os.listdir(nft_gallery_dir):
        item_path = os.path.join(nft_gallery_dir, item)
        if os.path.isdir(item_path) and item != "nft-gallery":  # Skip nested nft-gallery
            webp_path = os.path.join(item_path, "webp")
            if os.path.exists(webp_path):
                artists.append(item)
    
    print(f"Found {len(artists)} artists to convert")
    
    converted_count = 0
    for artist in sorted(artists):
        try:
            if create_simple_gallery(artist, preserve_flavor=True):
                converted_count += 1
        except Exception as e:
            print(f"Error converting {artist}: {e}")
    
    print(f"\\nCompleted: Converted {converted_count} out of {len(artists)} galleries to simple format")
